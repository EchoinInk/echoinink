...
const colorMap = {
  indigo: {
    main: 'rgb(var(--ei-midnight-rgb) / 0.03)',
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse 80% 50% at 50% 100%, rgb(var(--ei-midnight-rgb) / 0.4) 0%, transparent 60%),
            radial-gradient(ellipse 60% 40% at 50% 0%, rgb(var(--ei-midnight-rgb) / 0.3) 0%, transparent 50%)
          `,
          opacity: 0.6,
        }}
      />
...
export function AtmosphericBridge({
  ...
  const getGradient = () => {
    switch (fadeDirection) {
      case 'up':
        return `linear-gradient(to top, rgb(var(--ei-midnight-rgb) / ${start}) 0%, rgb(var(--ei-midnight-rgb) / ${end}) 50%, transparent 100%)`;
      case 'down':
        return `linear-gradient(to bottom, rgb(var(--ei-midnight-rgb) / ${start}) 0%, rgb(var(--ei-midnight-rgb) / ${end}) 50%, transparent 100%)`;
      case 'both':
      default:
        return `
          linear-gradient(to bottom, rgb(var(--ei-midnight-rgb) / ${start}) 0%, transparent 50%),
          linear-gradient(to top, rgb(var(--ei-midnight-rgb) / ${start}) 0%, transparent 50%)
        `;
    }
  };
...
export function AtmosphericVeil({
  ...
  const getBackground = () => {
    const baseColor = `rgb(var(--ei-midnight-rgb) / ${opacity})`;
