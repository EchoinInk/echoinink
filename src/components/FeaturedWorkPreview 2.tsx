...
import { EASE, DURATION, VIEWPORT, HOVER } from '@/lib/motion';
...
function FeaturedWorkCard({ work, delay = 0 }: { work: Work; delay?: number }) {
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0 transition-transform group-hover:scale-[1.025]"
        style={{
          background: work.gradient,
          backgroundColor: work.bg,
          transitionDuration: `${HOVER.duration.image}ms`,
          transitionTimingFunction: HOVER.easing.cinematic,
        }}
      />
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 md:p-7">
        ...
        <p
          className="font-structural text-[11px] md:text-[12px] text-white/30 leading-[1.78] mb-3 max-w-xs opacity-0 translate-y-1 transition-all group-hover:opacity-100 group-hover:translate-y-0"
          style={{
            transitionDuration: `${HOVER.duration.reveal}ms`,
            transitionTimingFunction: HOVER.easing.cinematic,
          }}
        >
          {work.description}
...
export function FeaturedWorkPreview() {
  return (
    <section className="relative ei-section py-14 md:py-24 overflow-hidden" aria-labelledby="featured-preview-heading">
      {/* Section header */}
      <div className="ei-container mb-8 md:mb-10">
        <div className="flex items-baseline justify-between">
          ...
          <Link
            to="/works"
            className="font-structural text-[10px] tracking-[0.25em] text-white/40 hover:text-white/70 transition-colors focus:outline-none focus:ring-2 focus:ring-white/20 focus:ring-offset-2 focus:ring-offset-[#0F1220] rounded-sm"
            style={{
              transitionDuration: `${HOVER.duration.text}ms`,
              transitionTimingFunction: HOVER.easing.standard,
            }}
          >
            View All
          </Link>
